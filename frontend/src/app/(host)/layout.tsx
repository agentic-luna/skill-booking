            ...
            <div className="flex flex-col items-center justify-center min-h-[60vh] p-6 text-center">
              <div className="bg-white border border-black/5 max-w-md p-10 rounded-[32px] shadow-[0_8px_30px_rgb(0,0,0,0.04)] space-y-8 animate-in zoom-in-95 duration-200">
                ...
                <div className="space-y-3">
                  <h2 className="text-2xl font-extrabold text-[#0b0c01]">Onboarding Required</h2>
                ...
                <div className="bg-black/[0.02] border border-black/5 rounded-3xl p-5 space-y-4 text-left">
                  <div className="flex items-center justify-between text-sm">
                    <div className="flex items-center space-x-3">
                      <span className={`w-3 h-3 rounded-full ${hasKycSubmitted ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]' : 'bg-amber-500'}`} />
                      <span className="font-semibold text-[#0b0c01]">KYC Identity</span>
                    </div>
                    <span className={`text-[10px] font-bold px-2.5 py-1 rounded-lg ${hasKycSubmitted ? 'bg-emerald-500/10 text-emerald-600' : 'bg-amber-500/10 text-amber-600'}`}>
                      {hasKycSubmitted ? 'Uploaded' : 'Pending'}
                    </span>
                  </div>
                  <div className="flex items-center justify-between text-sm">
                    <div className="flex items-center space-x-3">
                      <span className={`w-3 h-3 rounded-full ${hasBankDetailsSubmitted ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]' : 'bg-amber-500'}`} />
                      <span className="font-semibold text-[#0b0c01]">Bank Details</span>
                    </div>
                    <span className={`text-[10px] font-bold px-2.5 py-1 rounded-lg ${hasBankDetailsSubmitted ? 'bg-emerald-500/10 text-emerald-600' : 'bg-amber-500/10 text-amber-600'}`}>
                ...
                <div className="flex flex-col sm:flex-row gap-4 pt-4">
                  ...
                  {!hasBankDetailsSubmitted && (
                    <Link href="/host/earnings" className="flex-1">
                      <Button variant={hasKycSubmitted ? "default" : "outline"} className={`w-full text-sm font-bold py-6 rounded-2xl transition-all ${hasKycSubmitted ? 'bg-[#0b0c01] text-white hover:bg-black/80 shadow-lg' : 'bg-transparent border-black/10 text-[#0b0c01] hover:bg-black/5'}`}>
                        Link Bank Account
                      </Button>
                    </Link>
                  )}
