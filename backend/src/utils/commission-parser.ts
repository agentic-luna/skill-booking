...
export function parseCommissionRate(value: any): { commissionType: CommissionType; platformValue: number } {
  if (value === null || value === undefined) {
    return { commissionType: CommissionType.PERCENTAGE, platformValue: 15 };
  }

  // If value is a JSON object with type and value keys
  if (typeof value === 'object' && value !== null) {
    const type = value.type === 'FIXED' ? CommissionType.FIXED : CommissionType.PERCENTAGE;
  ...
  if (str.startsWith('FIXED')) {
    ...
    return { commissionType: CommissionType.FIXED, platformValue: isNaN(val) ? 15 : val };
  }
  ...
  if (str.startsWith('PERCENTAGE') || str.startsWith('PERCENT')) {
    ...
    const val = match ? parseFloat(match[1]) : 15;
    return { commissionType: CommissionType.PERCENTAGE, platformValue: isNaN(val) ? 15 : val };
  }
  ...
  if (!isNaN(numericVal)) {
    return { commissionType: CommissionType.PERCENTAGE, platformValue: numericVal };
  }
