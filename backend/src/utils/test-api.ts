...
async function runTests() {
  ...
  try {
    ...
    originalConfigs = await prisma.integrationConfig.findMany();
    ...
    await prisma.eventCommission.deleteMany({});
    await prisma.event.deleteMany({});
    await prisma.hostBankDetail.deleteMany({});
    ...
    if (!newLoginRes.success) console.log('DEBUG newLoginRes:', newLoginRes);
    ...
    const rawBankDetails = await prisma.hostBankDetail.findUnique({
    ...
    assert(eventCreateRes.success && eventCreateRes.data.status === EventStatus.PENDING, 'Event creation failed');
    ...
    const eventInQueue = queueRes.data.find((e: any) => e.id === eventId);
    ...
    assert(approveRes.success && approveRes.data.event.status === EventStatus.APPROVED, 'Admin event approval failed');
    ...
    const eventAfterCheckout = await prisma.event.findUnique({ where: { id: eventId } });
    ...
    assert(ledger && ledger.status === LedgerStatus.HELD, 'Ledger record missing or not HELD');
    ...
    const notifLog = await prisma.notificationLog.findFirst({
      where: { userId: clientSignupRes.data.user.id, channel: DeliveryChannel.EMAIL },
    });
    ...
    const ledgerAfterPayout = await prisma.transactionLedger.findFirst({
      where: { bookingId },
    });
    ...
    assert(cancelRes.data.booking.status === BookingStatus.CANCELED, 'Booking status was not updated to CANCELED');
    ...
    const eventAfterCancel = await prisma.event.findUnique({ where: { id: eventId } });
    ...
    const refundLedger = await prisma.transactionLedger.findFirst({
    ...
    if (!reviewRes.success) console.log('DEBUG reviewRes:', reviewRes);
...
async function post(path: string, body: any, token?: string, headers?: Record<string, string>) {
  return request('POST', path, body, token, headers);
}

async function put(path: string, body: any, token?: string, headers?: Record<string, string>) {
  return request('PUT', path, body, token, headers);
}

async function get(path: string, token?: string, headers?: Record<string, string>) {
  return request('GET', path, null, token, headers);
}

function request(method: string, path: string, body: any, token?: string, customHeaders?: Record<string, string>): Promise<any> {
...
import { commsService } from '../api/di-container';
...
server.listen(PORT, () => {
  ...
  workerInstance = startNotificationWorker(notificationRepo, commsService);
  runTests();
});
