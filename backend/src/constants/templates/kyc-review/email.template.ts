export interface KycReviewTemplateData {
  hostName: string;
  status: 'APPROVED' | 'REJECTED';
  rejectionReason?: string;
}

export function generateKycApprovedEmailTemplate(data: KycReviewTemplateData): string {
  return `
...
export function generateKycRejectedEmailTemplate(data: KycReviewTemplateData): string {
  return `
<div style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 24px; background-color: #ffffff; border: 1px solid #e5e7eb; border-radius: 16px;">
