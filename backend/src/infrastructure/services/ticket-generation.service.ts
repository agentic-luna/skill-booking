...
export class TicketGenerationService {
  ...
  async generateInvoicePdf(booking: any, hostHeader: string): Promise<Buffer> {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const itemGroups = new Map<string, { name: string; price: number; count: number; names: string[] }>();
        for (const p of participantsList) {
          ...
          group.count += 1;
          if (p.fullName) group.names.push(p.fullName);
        }
  ...
  async generateTicketPdf(booking: any, hostHeader: string): Promise<Buffer> {
    return new Promise(async (resolve, reject) => {
      try {
        const doc = new PDFDocument({ margin: 0, size: [400, 600] });
        const chunks: Buffer[] = [];
        ...
        const participantSummaryLines = participantsList.map((p: any) => {
  ...
  async generateTicketSvg(booking: any, hostHeader: string): Promise<string> {
    ...
    const attendeeDisplayName = (primaryAttendee?.fullName || `${client.firstName} ${client.lastName}`).toUpperCase();
    const participantNamesSummary = participantsList.map((p: any) => `${p.fullName}${p.ticketType?.name ? ` (${p.ticketType.name})` : ''}`).filter(Boolean).join(', ');
