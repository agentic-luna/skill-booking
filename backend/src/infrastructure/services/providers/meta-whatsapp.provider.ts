...
export class MetaWhatsAppProvider implements IWhatsAppProvider {
  ...
  async sendWhatsAppMessage(to: string, message: string): Promise<{ success: boolean; messageId?: string }> {
    ...
    let accessToken = env.META_WHATSAPP_ACCESS_TOKEN;
    let phoneNumberId = env.META_WHATSAPP_PHONE_NUMBER_ID;
    let apiVersion = env.META_WHATSAPP_API_VERSION || 'v20.0';
    ...
    if (isJson) {
      ...
      const templateKey = Object.keys(WHATSAPP_TEMPLATES).find(
        (key) => WHATSAPP_TEMPLATES[key].name === templateName
      );
      ...
      const config = WHATSAPP_TEMPLATES[templateKey];
